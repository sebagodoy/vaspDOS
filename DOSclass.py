...
class DOS:
    def __init__(self, Direction, Pol=True, Orb='spd', ml=False, **kwargs):
        ...
        if Orb not in ['s', 'sp', 'spd', 'spdf']:
            raise NameError(f"The orb option needs to be \'s\', \'sp\' or \'spd\'")
    ...
    # What curves are included
    def WhatCurves(self):
        print('  >> The object called \'' + self.Name + '\' includes the following curves')
        for i in self.DOS:
            print(' ' * 6 + '<> ' + i, end=' : ')
            for j in self.DOS[i]:
                print(j, end=' , ')
    ...
    def Add2(self, Colection):
        if isinstance(Colection, list):
            Colection.append(self)
        elif isinstance(Colection, dict):
            Colection[self.Name] = self
        else:
            raise NameError('The .Add2(colection) method adds the object to a colection that may be '
                            'a list or dictionary. Failed attemp for ' + self.Name)
    ...
    def UpdateScaleToElectronsFactor(self, iElectr):
        ...
        if self.Pol:
            self.GlobalFactor = self.Nelec / self.Integral(('global', 'sum'))
        else:
            self.GlobalFactor = self.Nelec / self.Integral(('global', '0'))
        # For the atom projected curves
        # Total scalling factor = n electrons/ integral from total DOS (sum of all atoms and orbitals)
        self.TotalFactor = self.Nelec / self.Integral(('total', 'sum'))
    ...
    def BandCenter(self, Index, FilledLevels=True, FermiCorrected=True, **kwargs):
        ...
        # Options:  FilledLevels = False        to include levels above fermi level (default case)
        #                                       if want only to the fermy level use FilledLevels = True
        ...
        # Report
        if kwargs.get('Report', False):
            print('Band center for '+ self.Name + ", curves " + str(Index), end='')
            if FermiCorrected:
                print(', fermi-corr.', end='')
            if FilledLevels:
                print(', including empty', end='')
            print(' : ' + "{:.4f}".format(BandCenter))
